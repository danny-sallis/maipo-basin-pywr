...
def make_model(contract_threshold_vals=-999999*np.ones(num_DP), contract_action_vals=np.zeros(num_DP),
               ...
               drought_status_agg="drought_status_single_day"):
    '''
    ...
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )
    # THIS IS DAILY, BUT WE WANT WEEKLY. WEEKLY CAUSED BUGS THOUGH
    model.timestepper = Timestepper(
        start=pd.to_datetime('2006-07-12'),  # start
    ...
    datestr = ["2006-07-12", "2011-07-06", "2016-06-29", "2021-06-23", "2026-06-17", "2031-06-11", "2036-06-05"]
    ...
    df = {
        ...
        "index_col": "Timestamp",
        "dayfirst": True}
    indicator_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # indicator_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    ...
    # drought_status_min_recent (lowest drought status in past four weeks)
    DroughtStatusAggregationParameter(
        model,
        dataframe=indicator_df,
        name="drought_status_min_recent",
        agg_func=lambda x: np.min(x),
        num_weeks=4,
        scenario=model.scenarios.scenarios[0]
    )
    paramIndex["drought_status_min_recent"] = model.parameters.__len__() - 1
    model.parameters._objects[paramIndex["drought_status_min_recent"]].name = "drought_status_min_recent"  # add name to parameter


    # drought_status_min_recent_class (lowest drought status in past four weeks)
    DroughtStatusMinAggregationParameter(
        model,
        dataframe=indicator_df,
        name="drought_status_min_recent_class",
        num_weeks=4,
        scenario=model.scenarios.scenarios[0]
    )
    paramIndex["drought_status_min_recent_class"] = model.parameters.__len__() - 1
    model.parameters._objects[paramIndex["drought_status_min_recent_class"]].name = "drought_status_min_recent_class"  # add name to parameter

    # drought_status_second_min_recent (second-lowest drought status in past twelve weeks)
    DroughtStatusAggregationParameter(
        model,
        dataframe=indicator_df,
        name="drought_status_second_min_recent",
        agg_func=lambda x: np.sort(x)[np.min([1, len(x) - 1])],
        num_weeks=12,
        scenario=model.scenarios.scenarios[0]
    )
    paramIndex["drought_status_second_min_recent"] = model.parameters.__len__() - 1
    model.parameters._objects[paramIndex["drought_status_second_min_recent"]].name = "drought_status_second_min_recent"  # add name to parameter


    # drought_status_threshold_passes (times a given threshold passed in recent time period)
    # Sets drought_status as very low after some number of passes and very high otherwise
    # Solution maybe isn't optimal, but does the job for now
    DroughtStatusAggregationParameter(
        model,
        dataframe=indicator_df,
        name="drought_status_threshold_passes",
        agg_func=lambda x: -np.infty if len(x[x < -0.5] >= 6) else np.infty,
        num_weeks=12,
        scenario=model.scenarios.scenarios[0]
    )
    paramIndex["drought_status_threshold_passes"] = model.parameters.__len__() - 1
    model.parameters._objects[paramIndex["drought_status_threshold_passes"]].name = "drought_status_threshold_passes"  # add name to parameter


    # drought_status_discounted_avg (weighted average of indicator with discount rate)
    # Solution maybe isn't optimal, but does the job for now
    agg_array = np.array([0.9**(11 - i) for i in range(12)])
    DroughtStatusAggregationParameter(
        model,
        dataframe=indicator_df,
        name="drought_status_discounted_avg",
        agg_func=lambda x: x.dot(agg_array[12 - len(x):12])/agg_array[12 - len(x):12].sum(),
        num_weeks=12,
        scenario=model.scenarios.scenarios[0]
    )
    paramIndex["drought_status_discounted_avg"] = model.parameters.__len__() - 1
    model.parameters._objects[paramIndex["drought_status_discounted_avg"]].name = "drought_status_discounted_avg"  # add name to parameter
    ...
    df = {
        ...
        "index_col": 0,
        "dayfirst": True}
    yeso_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # yeso_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # yeso_df = yeso_df.set_index('Timestamp')
    DataFrameParameter(
        model,
        dataframe=yeso_df,
        scenario=model.scenarios.scenarios[0],
    ...
    df = {
        ...
        "index_col": 0,
        "dayfirst": True}
    maipo_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # maipo_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # maipo_df = maipo_df.set_index('Timestamp')
    DataFrameParameter(
        model,
        dataframe=maipo_df,
        scenario=model.scenarios.scenarios[0],
    ...
    df = {
        ...
        "index_col": 0,
        "dayfirst": True}
    colorado_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # colorado_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # colorado_df = colorado_df.set_index('Timestamp')
    DataFrameParameter(
        model,
        dataframe=colorado_df,
        scenario=model.scenarios.scenarios[0],
        name="flow_Colorado"
    )
    ...
    df = {
        ...
        "index_col": 0,
        "dayfirst": True}
    volcan_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # volcan_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # volcan_df = volcan_df.set_index('Timestamp')
    DataFrameParameter(
        model,
        dataframe=volcan_df,
        scenario=model.scenarios.scenarios[0],
    ...
    df = {
        ...
        "index_col": 0,
        "dayfirst": True}
    laguna_negra_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # laguna_negra_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # laguna_negra_df = laguna_negra_df.set_index('Timestamp')
    DataFrameParameter(
        model,
        dataframe=laguna_negra_df,
        scenario=model.scenarios.scenarios[0],
    ...
    df = {
        ...
        "index_col": 0,
        "dayfirst": True}
    maipo_extra_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # maipo_extra_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # maipo_extra_df = maipo_extra_df.set_index('Timestamp')
    DataFrameParameter(
        model,
        dataframe=maipo_extra_df,
        scenario=model.scenarios.scenarios[0],
    ...
    extra_data_df = read_dataframe(model, df)#.sort_values('Timestamp')
    # extra_data_df['Timestamp'] = [x.datetime for x in Timestepper(
    #     start=pd.to_datetime('1990-08-01'),  # start
    #     end=pd.to_datetime('2036-05-28'),  # end
    #     delta=datetime.timedelta(7)  # interval
    # )]
    # extra_data_df = extra_data_df.set_index('Timestamp', drop=True)
    DataFrameParameter(
        model,
        dataframe=extra_data_df.squeeze(),
    ...
    NumpyArrayParameterRecorder(
        model,
        param=model.parameters["derechos_sobrantes_contrato"],
        temporal_agg_func="sum",
        agg_func="mean",
        name="derechos_sobrantes_contrato_recorder"
    )
    recorderIndex['derechos_sobrantes_contrato_recorder'] = model.recorders.__len__() - 1
