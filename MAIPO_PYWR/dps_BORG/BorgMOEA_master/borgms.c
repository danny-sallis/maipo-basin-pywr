...
void BORG_Master_shutdown(int rank) {
	...
	int rc = MPI_Send(NULL, 0, MPI_DOUBLE, rank, TAG_SHUTDOWN, comm);
...
void BORG_Master_send(int rank, BORG_Solution solution) {
	...
	int rc = MPI_Isend(solution->variables, solution->problem->numberOfVariables,
			MPI_DOUBLE, rank, TAG_EVALUATE, comm, &requests[rank-1]);
#else
	int rc = MPI_Send(solution->variables, solution->problem->numberOfVariables,
			MPI_DOUBLE, rank, TAG_EVALUATE, comm);
#endif
...
void BORG_Master_initialize(BORG_Algorithm algorithm) {
	...
	processing = (BORG_Solution*)calloc(commSize-1, sizeof(BORG_Solution));
...
void BORG_Master_loop(BORG_Algorithm algorithm) {
	...
	while ((remaining > 0) || 
		...
		MPI_Probe(MPI_ANY_SOURCE, TAG_RESULT, comm, &status);
		...
		if (((maxEvaluations <= 0) || (algorithm->numberOfEvaluations <= maxEvaluations-(commSize-1))) &&
				((maxHours <= 0.0) || (elapsedHours < maxHours))) {
...
BORG_Archive BORG_Master_run() {
	...
	BORG_Algorithm_set_minimum_population_size(algorithm, commSize-1 > 100 ? commSize-1 : 100);
	BORG_Algorithm_set_initial_population_size(algorithm, commSize-1 > 100 ? commSize-1 : 100);
...
void BORG_Worker_run() {
	...
	while (1) {
		...
		BORG_Timer_start("recv");
		rc = MPI_Recv(input, nvars, MPI_DOUBLE, 0, MPI_ANY_TAG, comm, &status);
		BORG_Timer_stop("recv");
		...
		if (rc != MPI_SUCCESS) {
			...
			BORG_Timer_start("send");
			rc = MPI_Send(output, nobjs+ncons, MPI_DOUBLE, 0, TAG_RESULT, comm);
			BORG_Timer_stop("send");
...
void BORG_Algorithm_ms_startup(int* argc, char*** argv) {
	...
	/* For master-slave all processes participate */
	comm = MPI_COMM_WORLD;
	commSize = worldSize;
	commRank = worldRank;
...
BORG_Archive BORG_Algorithm_ms_run(BORG_Problem _problem) {
	...
	if (commRank == 0) {
		...
		sprintf(name, "Worker %d", commRank);
